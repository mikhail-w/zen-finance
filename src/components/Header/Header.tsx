...
export default function Header() {
  useEffect(() => {
    ...
    // Define animateHeaderText variable
    const animateHeaderText = () => {
      const h1Text = document.querySelector('h1');
      const highlights = document.querySelectorAll(`.${styles.highlight}`);
      const fadeInElements = document.querySelectorAll(`.${styles.fadeIn}`);
      const fadeInLateElements = document.querySelectorAll(
        `.${styles.fadeInLate}`
      );
      ...
      if (h1Text) {
        h1Text.classList.add(styles.headerTitleVisible);
      }
      ...
      setTimeout(() => {
        ...
        if (heroImage) {
          heroImage.classList.add(styles.heroImageVisible);
        }
        ...
        setTimeout(() => {
          ...
          highlights.forEach((highlight, index) => {
            setTimeout(() => {
              highlight.classList.add(styles.highlightActive);
            }, index * 300);
          });
          ...
          setTimeout(() => {
            fadeInElements.forEach(el => {
              ...
              el.classList.add(styles.animationActive);
            });

            // Slightly later for fade-in-late elements
            fadeInLateElements.forEach(el => {
              (el as HTMLElement).style.animationDelay = '0.3s';
              el.classList.add(styles.animationActive);
            });
      ...
      <div className="flex-1 max-w-[150rem] w-full grid grid-cols-[45%_55%] gap-16 content-start justify-center items-center justify-items-start px-8 pt-[100px]">
        <div className="max-w-[90%]">
          <h1 className={styles.headerTitle}>
            The art of <span className={styles.highlight}>Finance</span>
            <br />
            with <span className={styles.highlight}>Effortless</span> simplicity
          </h1>
          <h4 className={styles.fadeIn}>
            Financial clarity, without the complexity.
          </h4>
          <button className={`btn--text btn--scroll-to ${styles.fadeInLate}`}>
            Learn more &#8595;
          </button>
        </div>
